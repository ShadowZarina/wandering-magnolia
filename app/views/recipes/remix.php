...
<main>
  <div class="container">
    <div class="page-header">
      <div>
        <a href="/recipe?id=<?= (int)$original['recipe_id'] ?>" style="font-size:.85rem; color:var(--pink);">← Back to Recipe</a>
        <h1 style="margin-top:8px;">Remix <span class="accent">Recipe</span></h1>
      </div>
    </div>

    <!-- Credit Banner -->
    <div class="remix-banner">
      <div class="remix-banner-icon"></div>
      <div>
        <div class="remix-banner-title">Remixing from <strong><?= htmlspecialchars($original['title']) ?></strong></div>
        <div class="remix-banner-sub">Make it your own — change the ingredients, directions, title, or image. Credit will be given automatically.</div>
      </div>
      <a href="/recipe?id=<?= (int)$original['recipe_id'] ?>" class="btn btn-ghost btn-sm">View Original</a>
    </div>
    ...
    <form class="add-recipe-page" method="POST" action="/remix-recipe" enctype="multipart/form-data">
      <input type="hidden" name="original_id" value="<?= (int)$original['recipe_id'] ?>">
        ...
        <div class="form-card">
          ...
          <div class="form-group">
            ...
            <input type="text" id="title" name="title"
                   value="<?= htmlspecialchars($original['title']) ?> (Remix)" required>
          </div>
          <div class="form-group">
          ...
          <div class="form-group">
            <label for="image">Recipe Image <span style="font-weight:400; color:var(--gray-500);">(leave blank to use original)</span></label>
            <div class="current-img-preview">
              <img src="<?= htmlspecialchars($original['image_url']) ?>"
                   alt="Original image"
      ...
      <div style="display:flex; gap:14px; padding-bottom:80px;">
        <button type="submit" class="btn btn-remix btn-lg">Publish Remix →</button>
        <a href="/recipe?id=<?= (int)$original['recipe_id'] ?>" class="btn btn-outline btn-lg">Cancel</a>
      </div>
